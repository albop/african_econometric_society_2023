...
# ╔═╡ e6db91a9-b40a-46a3-9160-f8dc36c36ba2
using Plots

# ╔═╡ 3916ec03-5ebb-4275-ac8c-8f5f951a0999
using Optim
...
md"""# Essential numerical tools and perturbation analysis  (1.c)
...
# ╔═╡ 55d1d974-2a73-42be-a3fa-846715ead255
md"""## Introduction
...
# ╔═╡ 0e482209-dcef-4f23-8781-b26751a2fcd9
md"""## A Tale of Two Optimization Problems

"""

# ╔═╡ 08678ea9-c399-423e-9f2f-2252b578c1ed
TwoColumn(
		md"""
...
# ╔═╡ bb42d425-a057-4b5e-be2a-ccd8ac1a12d9
md"""
...
# ╔═╡ 350ece58-d567-4ffb-952c-5ba3056e1edf
warning_box(md"""When using first order conditions to minimize a function one must pay attention to the two caveats:
- that the root does not correspond to a local *maximum*
- that the minimum is *global* (for $\forall x\in X$) not local
""")

# ╔═╡ 3f560d65-eb9b-44ea-bb7b-4cee64f97d56
Foldable("Local/Global Illustration", md"""
![Local vs Global Maximum](https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/Extrema_example_original.svg/1280px-Extrema_example_original.svg.png)
"""
)

# ╔═╡ f0cc7230-08cf-4be7-9625-715a60828a24
md"""
## Optimization tasks come in many flavours

"""

# ╔═╡ fdcd3001-47d8-4054-83e2-58c78ffb9c16
Foldable("Continuous versus discrete optimization",md"""
...
# ╔═╡ 6dda9f47-b2da-40b6-8157-bb2aaa5aa631
Foldable("""Constrained and Unconstrained optimization""", md"""
...
# ╔═╡ 8f84d0a3-5ac3-4590-a43a-03b75af15312
Foldable("Stochastic vs Deterministic",
md"""
...
# ╔═╡ efb31f8c-45a3-4662-b23e-71dc679f6a3e
Foldable("Local vs global Algorithms",
md"""
...
- We deal only with local algorithms, and consider local convergence properties.
  - ->then it might work or not
  - to perform global optimization just restart from different points.)
""")

# ╔═╡ 06508677-bdb6-4373-aa6b-3465bc492864
md"""## Math vs practice
...
# ╔═╡ 402c3ddb-71c9-4d15-8c63-6d4571452098
Foldable("A complicated surface", tip(md"""
Here is the surface (a cut) representing the objective that a deep neural network training algorithm tries to minimize.

![Is that C1?](https://www.mosphere.fr/eco309/slides/nonsmooth.png)

And yet, neural networks do great things!

"""))

# ╔═╡ 176fbed4-5c20-4010-986d-5bf5097d7ed3
md"""## What you need to know
...
- then you can just switch the various options, when you use a library...
  - root-finding: [Roots.jl](https://github.com/JuliaMath/Roots.jl) (1d) and [NLsolve.jl](https://github.com/JuliaNLSolvers/NLsolve.jl)
  - optimization: [Optim.jl](https://github.com/JuliaNLSolvers/Optim.jl)
  - more advanced modeling: [JuMP.jl](https://github.com/jump-dev/JuMP.jl) (comparable to GAMS)
"""

# ╔═╡ 1fced9b0-8947-4567-803d-ab6aaf7a8840
md"""## Unconstrained Optimization
...
# ╔═╡ bf71d540-6301-4473-a884-7bc381787403
md"""
## Quick terminology
...
# ╔═╡ f29e49ca-d8b7-4f43-961f-23785cd138e8
md"""## Newton-Raphson Root-Finding
...
# ╔═╡ dcd99d29-308b-4fbc-9d23-666a8bf01c68
Foldable("Newton Raphson Illustration", md"""![Newton-Raphson](https://upload.wikimedia.org/wikipedia/commons/8/8c/Newton_iteration.svg)
	"""
)

# ╔═╡ 55f48089-e0cc-4f6c-8a95-4b0ba67d80cb
md"""## Newton-Raphson Root Finding (2)
...
# ╔═╡ 210d296d-9f18-4517-ac31-8c1bdf4d5caa
md"""## Exercise 1: Simple Newton Algorithm

__Choose a two dimensional function with a zero.__
...
__Change the signature of the function `zero_newton(f::Function, x0::Vector{Float64}, backtracking=true)` and implement backtracking in each iteration.__

"""

# ╔═╡ 19566fca-e16c-4200-a2de-fd1538133ec8
function ϕ(x::Vector{Float64})
	y = [x[1]^2 + x[2]^2 - 0.5 , x[1] + x[2] - 1  ]
	dy = [ (2*x[1]) 1.0 ; 1.0 (2*x[1]) ]
	return (y, dy)
end

# ╔═╡ b6f5f907-1b73-4339-a432-52f0290b2a6d
ϕ([0.2, 0.1])

# ╔═╡ 041efe89-59e5-47b1-b017-9eb0809d0729
# what are the zeros of ϕ?

# ╔═╡ 4b69133c-f1a8-48c0-b707-84caf5287e27
function zero_newton(f::Function, x0::Vector{Float64})
end

# ╔═╡ d7e3e3bf-cad3-4473-94b9-612d35052128
md"""## Multidimensional Gradient Descent
...
# ╔═╡ b1840ead-1a33-482f-9831-4aed1263ffc0
Foldable("Gradient Descent Illustration", md"""![](https://www.mosphere.fr/eco309/slides/contours_evaluation_optimizers.gif)
""")

# ╔═╡ 38235a31-a1e1-4c3d-a7f8-fdc74698dbed
md"""## Multidimensional Newton Minimization
...
# ╔═╡ 702005cf-22f0-4dca-b0b4-ff7d7f5731db
Foldable("Newton Illustration",
	md"""![Multidimensional Newton](https://jermwatt.github.io/machine_learning_refined/mlrefined_images/math_optimization_images/Fig_2_11.png)""")

# ╔═╡ 57a2d454-bb74-4794-a05e-09e8665f2125
md"""Practical Problems
  - hessian $H(x_{n})$  is hard to compute efficiently
  - rather unstable
"""

# ╔═╡ 07fa813b-90a4-4352-82d7-c32adf7b873c
md"""
...
# ╔═╡ 0876dbbb-3295-44ca-b8bd-c193396116ae
md"""## Exercise 2: Profit optimization by a monopolist
...
__Write down the profit function of the monopolist and find the optimal production (if any) numerically. You can use the `Optim.jl` library.__
"""

# ╔═╡ e8f9baba-2935-4725-9d15-10b33f00b6f7
hint(md"""Try this code to define profits: 
```
p(q) = -2log(q/2)
c(q) = 0.5 + q*(1-q*exp(-q))
R(q) = q*p(q)
...
# ╔═╡ 7d6a8db5-5f15-4858-b2f7-927e56f9b863
begin
	p(q) = -2log(q/2)
	c(q) = 0.5 + q*(1-q*exp(-q))
	R(q) = q*p(q)
...
# ╔═╡ 4f39404c-c4f6-4926-af00-66f08d39b9ff
@bind q_sl Slider(0.001:0.01:  0.5)

# ╔═╡ 735644f6-974f-457b-83fd-c509e9663da1
profits(q_sl)

# ╔═╡ cf4134b2-79e0-4d42-ac37-9afa3bc95126
begin
	qvec = 0.001:0.01:  1.0
	πvec = profits.(qvec)
end

# ╔═╡ 4ce88942-9315-498c-8e80-bd6c51f5bb5c
plot(qvec, πvec)

# ╔═╡ ae53858e-f949-499a-a0d7-40ed81c12f50
res = optimize(u->-profits(u[1]) , [0.25])

# ╔═╡ 09906237-c7af-4d99-966f-552b34a2c65f
min0 = res.minimizer

# ╔═╡ 6d665833-4c62-4bad-a1c6-43e6dafa8d4f
println("Profit is maximum for q=$min0")

# ╔═╡ 7b6bbd92-f211-444b-b176-324df69e29f3
md"""## Dealing with constraints
...
subject to to the constraint $p_1 x_1 + p_2 x_2 \leq B$

where $U(.)$ is concave, $p_1$, $p_2$ and $B$ are given.
...
# ╔═╡ d12be4be-eb6f-4b6e-8cd9-6c315461166a
...
# ╔═╡ 6f1fa4c9-9e3b-477b-9b3f-067bbb601934
md"""
...
# ╔═╡ a1f23501-f7b0-412c-ab65-e994928435f4
md"""## Smooth method
...
# ╔═╡ a71b8a8c-1cda-4efa-b8af-f41ef3966a96
aside(tip(md"Fun fact: the formulation with a $\min$ is nonsmooth but also works quite often"))

# ╔═╡ 74958a24-35cf-4d80-bb93-b3fcde432ef7
md"""## Exercise 3: Constrained Optimization
Consider the function $f(x,y) = 1-(x-0.5)^2 -(y-0.3)^2$.

__Use Optim.jl to maximize $f$ without constraint. Check you understand diagnostic informations returned by the optimizer.__
"""

# ╔═╡ e94f658e-18fb-4fbd-9ec9-d5cf69f494ab


# ╔═╡ 18bb0544-8808-4047-b230-a839d7b842db
md"""__Now, consider the constraint $x<0.3$ and maximize $f$ under this new constraint.__"""

# ╔═╡ 12d93cba-6c9f-4238-9a5d-be2f5c3565c1


# ╔═╡ 68bfec70-3891-4ea9-8267-136f56873bd3
md"""__Reformulate the problem as a root finding problem with lagrangians. Write the complementarity conditions.__"""

# ╔═╡ 80117855-5187-4f68-9ddc-29914fdaaaec


# ╔═╡ 9af7a3ce-4dc0-4123-b738-fcf2462d3200
md"""__Solve using NLSolve.jl__"""

# ╔═╡ fbf38f23-a959-44e9-9e99-ca734742c338


# ╔═╡ 38abb4cc-3c89-4c74-86fe-b0f07db2ee6f
md"""__Adapt the `zero_newton` function from before to incorporate bound informations, and use the Fisher-Burmeister transform to solve the system.__"""

# ╔═╡ c0abd95f-e901-45ba-bcda-ea09993cb98c
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "cce6978b9bfd2be3e2039ac07f8d8aa0a96247cd"
